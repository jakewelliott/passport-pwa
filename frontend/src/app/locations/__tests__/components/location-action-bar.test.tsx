...
describe('LocationActionBar', () => {
    ...
    it('should contain coordinates link', () => {
        renderWithClient(<LocationActionBar park={mockPark} />);
        expect(screen.getByTestId('location-action-bar').innerHTML)
            .contains(`href="https://www.google.com/maps/place/${mockPark.coordinates.latitude},${mockPark.coordinates.longitude}"`);
    });

    it('should contain telephone link', () => {
        renderWithClient(<LocationActionBar park={mockPark} />);
        expect(screen.getByTestId('location-action-bar').innerHTML)
            .contains(`href="tel://${mockPark.phone}"`);
    });

    it('should contain website link', () => {
        renderWithClient(<LocationActionBar park={mockPark} />);
        expect(screen.getByTestId('location-action-bar').innerHTML)
            .contains(`href="${mockPark.website}"`);
    });

    it('should contain email link', () => {
        renderWithClient(<LocationActionBar park={mockPark} />);
        expect(screen.getByTestId('location-action-bar').innerHTML)
            .contains(`href="mailto:${mockPark.email}"`);
    });
});