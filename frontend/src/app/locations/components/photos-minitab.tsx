...
import type { Park, ParkPhoto } from '@/types';
    ...
    <>
      <div className='z-0 grid w-full grid-cols-3'>
        {park.photos.map((photo, index) => (
          <div key={photo.photo} className='aspect-square cursor-pointer' {...a11yOnClick(() => handleOpen(photo))}>
            <img
              data-testid={'photo'}
      ...
      {selectedPhoto && (
        <ImageModal photo={{ ...selectedPhoto, photoPath: `/photos/${selectedPhoto.photo}` }} onClose={handleClose} />
      )}
    </>
  );
};
