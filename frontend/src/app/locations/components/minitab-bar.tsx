...
export const LocationMiniTabBar: React.FC<LocationMiniTabBarProps> = ({ children }) => {
    ...
    return (
        <>
            <div className='flex h-12 items-center justify-around border-supporting-bluegray border-b'>
                {['Details', 'Photos', 'Notes'].map((tab, index) => (
                    <div
                        ...
                        className={`relative flex h-full w-full cursor-pointer items-center justify-center p-2 ${
                            choice === index ? 'text-system-black' : 'text-supporting-darkgray'
                        }`}
                    >
                        <p>{tab}</p>
                        <div
                            className={`absolute right-0 bottom-0 left-0 h-0.5 bg-secondary-darkteal ${choice !== index ? 'hidden' : ''}`}
                        />
                    </div>
                ))}
            </div>
