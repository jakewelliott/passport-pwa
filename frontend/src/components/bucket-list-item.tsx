...
const CompletedAtView = ({ updatedAt }: { updatedAt: string }) => (
  <div className='flex flex-wrap items-center justify-end gap-2'>
    <p className='p-mini'>{DateHelper.toStringShort(new Date(updatedAt))}</p>
  </div>
);
    ...
    <div key={item.id} className='my-2.5 flex items-start' {...a11yOnClick(handler)}>
      ...
      <div className='flex w-full flex-col justify-center'>
        ...
        <div className='flex flex-wrap items-center justify-between gap-2'>
          ...
          {isCompleted && <CompletedAtView updatedAt={completion?.updatedAt} />}
        </div>
      </div>
    </div>
  );
