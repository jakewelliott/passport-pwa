...
export const StampsOverview = () => {
  ...
  return (
    <div className='mx-auto my-4'>
      {selectedPark ? <StampDetails code={selectedPark.code} handleClose={() => setSelectedPark(null)} /> : null}
      {sortedParks.map((park: Park) => (
        <div className='mx-9 my-4 flex justify-evenly' key={park.code}>
          <button key={park.code} onClick={() => setSelectedPark(park)} className='mx-3' type='button'>
            <img
